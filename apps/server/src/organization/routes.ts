...
export async function registerOrganizationRoutes(app: FastifyInstance, service: OrganizationService, authenticate: preHandlerHookHandler): Promise<void> {
  ...
  app.patch("/api/organization/members/:membershipId/status", { preHandler: [app.csrfProtection, authenticate, manageMembers] }, async (request, reply) => { const { membershipId } = membershipParams.parse(request.params); const { status } = memberStatusSchema.parse(request.body); try { return { membership: await service.setMemberStatus(request.auth!, membershipId, status) }; } catch (error) { if (error instanceof OrganizationConflictError) return reply.code(409).send({ error: "organization_conflict", message: error.message }); throw error; } });
