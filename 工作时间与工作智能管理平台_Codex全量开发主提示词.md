...
14. AI 使用可配置 GLM Provider，默认 `ZHIPU_MODEL=glm-5.3-flash`；采用轻量实时 + 异步自动 + 手动深度分析混合模式。
...
手机号验证/SMS 采用 Provider Interface；没有 SMS 凭据时开发环境提供安全 mock，但生产启动必须明确告警，不得假装发送成功。
