...
员工可在“项目”目录加入项目，再打开具体节点，在“协作者与负责人”中点击“认领此节点”。认领保留原协作者，不会覆盖已有负责人或授予项目管理权限。

添加工作记录时，可跨项目勾选多个关联节点；主节点仅从已选节点中选择，用于归集工时。每个已选节点都可以独立勾选进度更新，通过滑块、数字输入或 0%、25%、50%、75%、100% 快捷按钮填写完成度。自动汇总阶段可直接添加其具体子任务，阶段进度继续由系统汇总。工时和全部节点更新在同一事务中完成，未勾选节点不改动，时长不重复计算。编辑旧草稿默认不重放历史完成度；计划不更新项目进度。

新建记录附带文件时，工作草稿先保存，随后上传并核验文件；失败项会保留在当前页面并提供“重试未完成证据”。文件已经上传但核验暂时失败时，只重试核验。离开或刷新页面会释放本地文件，届时可在对应记录的“证据”中重新选择文件。仅已完成核验且审核人可见的证据可用于提交审核。

## 工程布局

全模块复验执行 `pnpm check`、`pnpm test:e2e` 和 `pnpm test:live`。真实链路验收使用独立临时数据库和本地文件服务，实际加载构建后的前端与正式 API，覆盖 Chromium 五种尺寸、Firefox 桌面与 WebKit 手机尺寸；首次运行需执行 `pnpm exec playwright install chromium firefox webkit`。截图及页面控件清单位于 `test-results/live` 和 `test-results/engines`，CI 会保留这些验收产物 7 天。验收范围和外部依赖边界见 [验收记录](docs/acceptance-2026-09-09.md)。
