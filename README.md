...
- 团队成员只能由有成员管理权限的人通过邮箱或 E.164 手机号加入白名单。每个成员可在 **账户安全** 增加并验证另一种登录/找回方式；未验证的标识绝不能登录或重置密码，且系统禁止移除最后一个已验证标识。邀请、绑定验证与密码重置仅通过已配置的 SMTP 或 Twilio 真实投递；Owner 邀请的未配置通道会明确失败且不创建无效白名单，公共密码重置则始终使用通用回执防止枚举账号，所有新一次性链接使用 URL fragment，避免令牌进入普通访问日志。
