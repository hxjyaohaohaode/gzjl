...
- 团队成员只能由有成员管理权限的人通过邮箱、E.164 手机号或两者同时加入白名单。默认会只向当前授权管理员显示一次可复制的邀请链接，由管理员通过私密渠道人工交付，因此首次部署无需 SMTP 或短信服务；若显式选择自动邮件/短信投递，未配置的通道会在创建前明确失败。成员接受一次邀请后，填写的全部联系方式都会验证为同一账号的登录方式。每个成员也可在 **账户安全** 增加并验证另一种登录/找回方式；未验证的标识绝不能登录或重置密码，且系统禁止移除最后一个已验证标识。公共密码重置仍使用已配置的真实渠道和统一回执防止枚举；无外部通道时，唯一 Owner 可在成员详情完成当前密码（及已启用的 TOTP）二次验证后，生成一次性手工重置链接。所有新一次性链接都把令牌放在 URL fragment，避免令牌进入普通访问日志。
