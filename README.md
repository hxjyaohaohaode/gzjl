...
- 授权用户可创建 CSV、JSON、XLSX 或 PDF 工时后台导出。任务由 Worker 从服务端权限快照读取真实事实，显示进度并支持取消/失败重试；文件写入同一私有 S3/R2 桶、下载前再次鉴权，短时签名且 24 小时后由 Worker 清理。电子表格文本会阻断公式注入，PDF 嵌入中文字体；任务完成或失败会产生站内/可选 Push 通知和审计记录。
