# 权限与数据范围

## 结论

权限判断只在 API 服务端进行。前端的菜单隐藏、路由守卫和筛选器仅改善使用体验，不能作为授权依据。每个授权由 `permission + scopeKind + scopeId` 组成；组织级授权可覆盖下级范围，项目和组织单元授权只匹配明确的同一对象。

## 内置职责

|职责|典型能力|数据范围|
|---|---|---|
|Owner|组织、成员、角色、薪资配置和结算、审计|组织|
|Manager|项目维护、工时审核、团队分析、范围内导出|组织单元或项目|
|Member|本人计时、手工补录、证据、查看个人薪资|self|

真正生效的能力来自角色的 grant，而不是角色名称。Owner 可以用 `roles.manage` 将单项能力授予项目或组织单元，并在成员停用时撤销授权。

## 系统角色目录与邀请默认值

首次初始化会在同一事务中创建 `Owner`、`Manager` 和 `Member` 三个系统角色；不能只创建不可通过邀请授予的 Owner。早期版本已经建立的组织会在下一次读取组织角色目录或发起邀请时，以幂等方式补齐缺失的系统角色和缺失权限，不会覆盖自定义角色或删除既有授权。

白名单邀请默认预选最小权限的 `Member`，Owner 必须显式切换后才能发送 `Manager` 或其他可分配角色。若角色目录正在恢复或异常为空，界面不会提交一个没有访问角色的邀请；服务端对没有携带角色 ID 的旧客户端同样只会安全降级到 `Member`。

邀请可登记邮箱、E.164 手机号或两者，并要求至少有一种身份。默认只向当前有成员管理权限的操作者显示一次可复制的激活链接，操作者须通过私密渠道人工传递；重发会撤销该成员先前所有未使用链接。只有明确选择自动投递且对应服务已配置时，系统才会使用邮件或短信渠道。

## 关键权限

|权限|用途|
|---|---|
|`project.create` / `project.manage`|创建项目、维护节点、分支、边和版本|
|`work.view_own` / `work.view_project_public` / `work.view_full_scope`|本人、项目公开或完整范围内的工时读取|
|`work.review`|提交审核决定；服务端禁止审批人审批自己提交的记录|
|`evidence.view_management`|读取管理范围证据；私有证据不会因该权限而公开|
|`payroll.view_own` / `payroll.view_scope` / `payroll.configure` / `payroll.settle`|个人薪资、范围薪资、规则配置与锁定结算|
|`analytics.view_team` / `ai.team_analysis`|范围内聚合分析与 AI 洞察请求|
|`audit.view` / `export.scope`|审计查询和按既有范围过滤后的导出|

## 员工自主加入与认领

有效组织成员可在项目目录加入本组织未归档的项目，获得普通项目成员身份。加入不会授予 `project.manage`。加入后，在节点详情的“协作者与负责人”中可认领尚无主要负责人的节点；服务端只允许认领者指定自己，保留原有协作者，并检查组织、成员有效状态、项目成员关系、分支状态与节点版本。已完成、已取消、已删除及归档分支中的节点不可新认领；已有其他负责人时须由管理者调整分工。认领写入节点版本、活动与审计，并产生实时更新事件。

员工填写真实工作记录时可同步关联主节点的手动完成度；自动汇总节点需选择其可手动更新的子节点，保存后由服务端重新汇总。移动端提供数值输入、滑块及快捷百分比按钮。

## 数据收敛顺序

1. 身份验证取得会话中的组织、成员和 grant 快照。
2. 服务端先将 grant 编译为组织单元、项目或本人范围的 SQL 条件。
3. 数据查询应用该条件，再应用日期、状态等业务筛选。
4. 对内容、结果和证据再应用字段可见性；无权字段不会在响应中返回。
5. 导出、AI、看板和实时事件复用同一范围逻辑，不另建“管理员全量读取”的旁路。

## 证据的特殊规则

证据可见性为 `private`、`management_only` 或 `project_visible`。提交审批至少需要一项 `management_only` 或 `project_visible` 且已核验的证据；有效审核人仅在该记录待审期间、并且其 `work.review` scope 覆盖该记录时读取这些证据，`private` 证据始终只对提交人开放。对象存储 bucket 永不公开；文件直传和下载均为短时预签名 URL。替换与删除写审计日志，替换前的哈希、对象键和版本快照保存在 `attachment_versions`，以便追踪，不会被常规列表返回。
